...
function startServer() {
  ...
  // ===== Temporary Test Data Endpoint =====
  app.post('/api/admin/test-data', authMiddleware, adminMiddleware, async (req, res) => {
    try {
      await runQuery("DELETE FROM registrations");

      const clubs = await getAll("SELECT club_id, club_name FROM clubs WHERE is_admin = 0 ORDER BY club_id");
      if (clubs.length === 0) return res.status(400).json({ error: '無社團資料，請先匯入社團' });

      await runQuery("UPDATE settings SET value = '1' WHERE key = 'current_phase'");
      await runQuery("UPDATE settings SET value = '10' WHERE key = 'guaranteed_quota'");
      await runQuery("UPDATE settings SET value = '160' WHERE key = 'phase1_total_quota'");

      const positions = ['理事', '監事', '聘任幹部', '眷屬'];
      const surnames = ['王', '李', '張', '陳', '林', '黃', '吳', '劉', '蔡', '楊', '許', '鄭', '謝', '郭', '洪', '曾', '邱', '廖', '賴', '徐'];
      let nameIdx = 0;
      const clubResults = [];

      // 前5社各15人（10保障 + 5候補）
      for (let i = 0; i < 5 && i < clubs.length; i++) {
        const club = clubs[i];
        for (let j = 0; j < 15; j++) {
          const name = surnames[nameIdx % surnames.length] + ' test' + (nameIdx + 1);
          const position = positions[j % positions.length];
          const mealType = j % 2 === 0 ? '葷' : '素';
          const status = j >= 10 ? 'standby' : 'registered';
          await insert(
            "INSERT INTO registrations (club_id, position, name, id_card, birthday, phone, meal_type, phase, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [club.club_id, position, name, 'A' + String(nameIdx + 1).padStart(9, '0'), '2000-01-01', '090000' + String(nameIdx).padStart(3, '0'), mealType, 1, status]
          );
          nameIdx++;
        }
        clubResults.push({ club_id: club.club_id, club_name: club.club_name, registered: 10, standby: 5, total: 15 });
      }

      // 其餘社團分配60人
      const remaining = 120 - 75;
      const otherClubs = clubs.slice(5);
      const base = Math.floor(remaining / otherClubs.length);
      const extra = remaining % otherClubs.length;

      for (let i = 0; i < otherClubs.length; i++) {
        const club = otherClubs[i];
        ...
        for (let j = 0; j < count; j++) {
          ...
          await insert(
            "INSERT INTO registrations (club_id, position, name, id_card, birthday, phone, meal_type, phase, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [club.club_id, position, name, 'A' + String(nameIdx + 1).padStart(9, '0'), '2000-01-01', '090000' + String(nameIdx).padStart(3, '0'), mealType, 1, 'registered']
          );
          nameIdx++;
        }
        if (count > 0) clubResults.push({ club_id: club.club_id, club_name: club.club_name, registered: count, standby: 0, total: count });
      }
      ...
      res.json({ message: '已建立測試資料', summary });
    } catch (err) {
