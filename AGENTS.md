...
- `POST /api/admin/announce/sync` — 同步群組成員（admin；`syncGroupMembers()`：對每個 group 來源分頁拉 `GET /v2/bot/group/{id}/members/ids` 全部成員 → 逐個 `GET /v2/bot/profile/{userId}` 取名稱 → upsert 進 `line_sources` 為 user（`ON CONFLICT DO UPDATE SET source_name`，不動 `last_message_at`）；**只有傳過訊息給 bot 的人才會被 `line_sources` 收錄**，群組內從未傳訊息的成員靠此同步後才能被 announce/match 比對；無 LINE token 回 `enrolled:0`）
