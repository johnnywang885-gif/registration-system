...
- **診斷歷史（2026-08-24）**：8/23 新增報名未公告的排查結論——功能自 8/20 上線起在正式環境**從未成功推送過一次**（settings 無 `stats_announce_date` 可證）；程式邏輯正常、部署正常，根因為 LINE push 持續失敗（最可能：免費額度耗盡或 bot 已離開主群組）。上述可觀測性與測試推播即為此而加——部署後到後台按「測試推播」看 detail 即可分辨（429＝額度、400/404 group＝bot 退群需重新邀請）。公開 `/api/summary` 的 settings 已過濾所有 `stats_*` 鍵（防洩漏內部錯誤細節）。
