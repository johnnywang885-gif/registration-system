...
- `review_k_stats_announce.js` — boots real server on PORT 34916, guards the 報名進度自動公告 stack: `dayMultiple5()` 單元（5/10/15/20/25/30 為 true、31/其他/壞格式為 false）、`buildStatsMessage()` 內容（各階段人數＋繳費社團數＋名額/剩餘）、無 LINE 設定時 `sendStatsAnnounce` 優雅 false 且不寫 `stats_announce_date`、`stats_announce=off` 停用、當日已公告過則 periodic 去重、settings PUT 白名單接受 `stats_announce`、HTTP 報名異動不中斷、`runEnforcement()` 回傳 `changed` 旗標（未來截止日 false、逾期棄權 true）。
...
- 公告內容：第一階段已報名（待繳費）/已繳費/候補/棄權人數＋已繳費社團家數＋第二階段人數＋總名額/正取/剩餘（`buildStatsMessage()` 即時查 DB）。
- 開關：`settings.stats_announce`（`on` 預設／`off` 停用），後台 系統設定 tab 勾選框控制；未設定＝啟用。推送失敗（LINE 未設定等）不會記錄 `stats_announce_date`，隔 30 分鐘會再試；`pushToGroup` 未設定時回 false 不拋錯。
